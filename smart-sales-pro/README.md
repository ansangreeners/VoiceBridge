...
## 주요 기능
- 거래처 추가/삭제 (일보 탭 상단 ＋ 거래처 추가 버튼) — 병원명·담당의·진료과·키워드 등록
- 새 일보 작성 (헤더 ✍️ 새 일보 버튼) — 거래처 선택 후 면담 메모 저장, 거래처 카드에 즉시 반영
- 면담 1분 전 브리핑 모달 — 담당의 다수 등록 시 탭으로 전환
- 경비 기록/삭제, 월별 합계
- Live 미연동 시 위 모든 데이터는 localStorage에 저장되어 새로고침에도 유지됨
